...
/*   Updated: 2023/05/27 00:57:09 by wbouwach         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	define_vars(int *len, int *i, int *flag, char *line)
{
	*len = 0;
	*i = 0;
	*flag = 0;
...
static	char	*parse_operator2(char *line)
{
	...
	cmd = define_vars(&len, &i, &flag, line);
	free (line);
	while (cmd[i])
	{
...
char	*parse_operator(char *line)
{
	...
	while (cmd[i])
	{
		...
		{
			if (i > 1 && ft_strlenn(cmd) > 1
				&& cmd[i - 1] != ' ' && !is_redirection(cmd[i - 1]))
